...
# test predictions
#changed to no longer use test predictions
newProbs <- predict(model, newdata = testData, type = "response")
predictions <- as.numeric(colnames(newProbs)[apply(newProbs, 1, which.max)])
...
par(mfrow = c(2, 1))
hist(trainData$anxiety, main="Actual Anxiety", xlab="Anxiety")
hist(predictions, main="Predicted Anxiety", xlab="Anxiety")

# CONFUSION MATRIX (maybe fixed???)

conMat <- data.frame(matrix(0, ncol = 4, nrow = 4))
colnames(conMat) <- c("1", "2", "3", "4")
rownames(conMat) <- c("1", "2", "3", "4")

# use testData$anxiety instead of anxiety
#comparing test predictions to the test dataset’s anxiety values, instead of the full dataset

actual <- testData$anxiety

for (i in 1:length(predictions)) {
  conMat[actual[i], predictions[i]] <- conMat[actual[i], predictions[i]] + 1
}
